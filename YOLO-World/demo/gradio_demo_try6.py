...
def run_image(image, text, max_num_boxes, score_thr, nms_thr):
    """
    Perform detection on a single image using YOLO-World model.
    """
...
def webcam_detection(text, max_num_boxes, score_thr, nms_thr):
    """
    Capture a frame from the webcam, process it, and return annotated image.
    """
    ...
    if not ret:
        return np.zeros((480, 640, 3), dtype=np.uint8)  # Return black image if no frame

    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    pil_image = Image.fromarray(frame_rgb)
    return run_image(pil_image, text, max_num_boxes, score_thr, nms_thr)
...
def create_gradio_interface():
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            webcam_button = gr.Button("Start Webcam Detection")
