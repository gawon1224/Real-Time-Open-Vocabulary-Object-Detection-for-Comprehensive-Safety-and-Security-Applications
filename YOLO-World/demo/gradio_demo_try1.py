...
def run_image(runner,
    ...
    #data_info = dict(img_id=0, img=np.array(image), texts=texts) 기존 
    # print(texts)
    ...
    # 여기에서 객체 정보 추출
    out_bboxes = pred_instances.bboxes  # (N, 4) 형태
    out_labels = pred_instances.labels  # (N, ) 형태
    # print(out_labels)

    # 객체별 정보 저장
    objects_info = []
    for bbox, label, score in zip(out_bboxes, out_labels, pred_instances.scores):
        obj_info = {
            'class_id': label.item(),
            'class_name': texts[label.item()],
    ...
    # 객체 정보 CSV로 저장
    save_objects_info_to_csv(objects_info)
...
def demo(runner, args):
    with gr.Blocks(title="YOLO-World") as demo:
        with gr.Row(): #YOLO-World: Real-Time Open-Vocabulary
            gr.Markdown('<h1><center>Intelligent Robot Project '
                        'YOLO-World: Real-Time Open-Vocabulary</center></h1>')
        with gr.Row():
        ...
        # # 사용자 입력 값 (클래스 리스트)을 가져오는 부분
        # def get_class_names(text):
        #     # 텍스트박스에서 입력된 클래스 리스트를 가져와 처리
        ...
        # 클래스 리스트가 수정되면 get_class_names()를 호출하여 입력된 텍스트를 처리하고, 이 값을 이후 함수에서 사용할 수 있도록 처리
        # input_text.submit(get_class_names, inputs=input_text, outputs=None)
